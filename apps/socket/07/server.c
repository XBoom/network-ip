...
int main(int argc, char *argv[])
{
    ...
    server_addr = socket(AF_INET, SOCK_DGRAM, 0);
    CHECK_RET(server_addr == -1, "socket failed");
    ...
    sockaddr_size_t client_addr_len = sizeof(client_addr);
    while(1)
    {
        ...
        LOG_INFO("recv message %s from %s", message, inet_ntoa(client_addr));

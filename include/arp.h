...
#define ARP_HDR_LEN sizeof(arp_hdr_st)
#define ARP_DATA_LEN sizeof(arp_ipv4_st)
...
typedef struct arp_hdr_st
{
    ...
    unsigned char data[];   
}__attribute__((packed)) arp_hdr_st;
...
}__attribute__((packed)) arp_ipv4_st;

//ARP缓存
typedef struct arp_cache_entry_st
{
    uint16_t hwtype;
    uint32_t sip;
